...
// Haal de 4 goedkoopste actieve vluchten op voor de homepage
$flights = $pdo->query('SELECT * FROM flights WHERE active = 1 ORDER BY price ASC')->fetchAll();

// Haal de 3 meest recente goedgekeurde reviews op voor de homepage
...
<body class="bg-[#f8f5ef]">
  ...
  <section class="py-20 px-6">
    <div class="max-w-6xl mx-auto">
      <h2 class="text-2xl md:text-3xl font-bold text-[#2e5435] mb-2" style="font-family:'Kadwa',serif;">Popular
        Destinations</h2>
        ...
        <?php foreach ($flights as $f): ?>
          <a href="flight-detail.html"
            ...
            <div class="p-5 flex flex-col flex-1">
              ...
              <div class="text-gray-400 text-xs mb-4"><?= htmlspecialchars($f['airline']) ?> ·
                <?= date('d M Y', strtotime($f['departure_date'])) ?></div>
              <div class="flex items-center justify-between mt-auto">
                <span class="font-bold text-[#2e5435]">from €<?= number_format($f['price'], 0, ',', '.') ?></span>
                <span
                  class="text-xs border border-[#2e5435] text-[#2e5435] rounded-lg px-3 py-1 hover:bg-[#2e5435] hover:text-white transition-colors">View
                  →</span>
              </div>
            </div>
          </a>
  ...
  <section class="bg-[#1e3b24] py-20 px-6">
    <div class="max-w-6xl mx-auto">
      <h2 class="text-2xl md:text-3xl font-bold text-white mb-2" style="font-family:'Kadwa',serif;">The HighFlights
        Standard</h2>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <?php foreach (
          [
            ['icon' => '✈️', 
            'title' => 'Cannabis-friendly flights',
             'desc' => 'Every flight we book allows in-flight cannabis consumption. Exhale before you even land.'],
            ['icon' => '🌿', 'Pre-rolls included', 'Sourced from growers we know personally. Rolled the morning of departure. Matched to your destination.'],
            ['🗺️', 'Curated destinations', 'Slow cities, quiet coasts, retreats that don\'t require an itinerary. You pick the place, we handle the rest.'],
          ]
          as $flight): ?>

          <div class="border-l-4 border-[#a3c7a7] pl-6">
            <div class="text-3xl mb-4"><?= $flight[0] ?></div>
            <h3 class="text-white font-bold text-lg mb-2"><?= $flight[1] ?></h3>
            <p class="text-[#a3c7a7] text-sm leading-relaxed"><?= $flight[2] ?></p>
          </div>
        <?php endforeach; ?>
      </div>
    </div>
  ...
  <section class="py-20 px-6">
    <div class="max-w-6xl mx-auto">
      <h2 class="text-2xl md:text-3xl font-bold text-[#2e5435] mb-2" style="font-family:'Kadwa',serif;">What travelers
        say</h2>
          ...
          <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 flex flex-col gap-3">
            <div class="flex items-start justify-between">
              <div>
                ...
                <div class="text-gray-400 text-xs"><?= date('M Y', strtotime($r['created_at'])) ?> ·
                  <?= htmlspecialchars($r['destination_name']) ?></div>
              </div>
              <div class="text-yellow-400 text-sm">
                <?= str_repeat('★', $r['rating']) . str_repeat('☆', 5 - $r['rating']) ?></div>
            </div>
  ...
  <!-- ── Footer ── -->
  <footer class="bg-[#1e3b24] text-[#a3c7a7] py-16 px-6">
    <div class="max-w-6xl mx-auto">
      <div class="grid grid-cols-1 md:grid-cols-4 gap-10 mb-10">
        <div class="md:col-span-1">
          <div class="text-white font-bold text-lg mb-3" style="font-family:'Kadwa',serif;">🌿 HighFlights</div>
          <p class="text-sm leading-relaxed">A travel agency that partners exclusively with cannabis-friendly airlines.
          </p>
        </div>
        <div>
          <h4 class="text-white font-bold text-sm uppercase tracking-wide mb-4">Navigate</h4>
        ...
        <div>
          <h4 class="text-white font-bold text-sm uppercase tracking-wide mb-4">Account</h4>
          <ul class="space-y-2 text-sm">
            <li><a href="login.php" class="hover:text-white transition-colors">Log in</a></li>
            <li><a href="register.php" class="hover:text-white transition-colors">Register</a></li>
            <li><a href="account.php" class="hover:text-white transition-colors">My account</a></li>
          </ul>
        </div>
        <div>
          <h4 class="text-white font-bold text-sm uppercase tracking-wide mb-4">Legal</h4>
          <ul class="space-y-2 text-sm">
      ...
      <div class="border-t border-[#2e5435] pt-6 flex flex-col md:flex-row justify-between text-xs gap-2">
